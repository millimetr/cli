...
const core = async ({ static, output, routes, transformInternals, globals }) => {
    ...
    await routes.map(async (singleRoute) => {
        ...
        const internals = { routes: { all: routesSummary, active: { title: singleRoute.title, url: singleRoute.url }, globals } };
        const compiled = await compileEjs(templatePath, { ...singleRoute, millimetr: transformInternals ? transformInternals(internals) : internals });
